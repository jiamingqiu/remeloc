...
idx_glm_optns <- function(optns){
  ...
  res <- which(!(names(optns) %in% c(
    # computation related
    'n.local', 'method.trim', 'local.reach', 'proj.pd',
...
#'        Currently \code{2} is only ad hoc, one should not use its estimate.
#'    }
...
#'        whether to project the estimated metric tensor to positive definite.
#'        Projection via truncate negative eigenvalues.
#'    }
#' }
#'
#' @family {locMetric}
...
set_optns <- function(optns = list(), ...){
  ...
  if(!is.null(optns$proj.pd))
    stopifnot(inherits(optns$proj.pd, 'logical'))
  else
    optns$proj.pd <- FALSE
...
fitMetric <- function(formula, data, coord, optns = list()){
  ...
  stopifnot(all(is.finite(
    # as.matrix(data)
    as.matrix(get_all_vars(formula, data))
  )))
...
estiMetric <- function(target, model, resp, obsv.coord, optns = list()){
  ...
  if(is.null(optns$constructor))
    optns$constructor <- list(metric = termMetric(d = d))
  if(optns$deriv >= 1 & is.null(optns$constructor$pre.christ))
    optns$constructor$pre.christ <- termPreChrist(d = d)
  if(optns$deriv >= 2 & is.null(optns$constructor$curvature))
    optns$constructor$curvature <- termRiemCvt(d = d) #TBD
  # }else{
  ...
  # # the diff in coord to the target points
  # cd.target <- lapply(seq_along(target.nbhd), function(idx.target){
  #   tm <- obsv.coord[target.nbhd[[idx.target]], ]
  #   tm <- tm - target[idx.target, col(tm)]
  #   rownames(tm) <- as.character(target.nbhd[[idx.target]]) # for later slicing
  #   return(tm)
  # })
  ...
  # }else{ # if slice here, sample per endpoint, not good
  ...
  # # recompute the diff in coord to the target points w/ working subset
  # cd.target <- lapply(seq_along(target.nbhd), function(idx.target){
  #   tm <- obsv.coord[target.nbhd[[idx.target]], ]
  #   tm <- tm - target[idx.target, col(tm)]
  #   rownames(tm) <- as.character(target.nbhd[[idx.target]]) # later slicing
  #   return(tm)
  # })
  # names(cd.target) <- names(target.nbhd)
  ...
  loc.obsv <- lapply(target.nbhd, function(nbhd){
    ...
    # random slicing (per unit now)
    if(nrow(loc.graph) > max.n.local & optns$method.trim == 'random'){
      idx.use <- sort(sample.int(n = nrow(loc.graph), size = max.n.local))
      loc.graph <- loc.graph[idx.use, , drop = F]
    }
  ...
  if(!all(n.loc.obsv >= min.n.local)) {
    warning(sprintf(
      'samples are not dense enough near %s target point(s).',
      sum(n.loc.obsv < min.n.local)
    ))
  }

  # # takes too much RAM
  # # reformat the diff in coord to the target points for faster slicing
  # template <- rep(list(NULL), nrow(obsv.coord))
  # cd.target <- lapply(cd.target, function(cd.mat){
  #   tm <- asplit(cd.mat, 1)
  #   names(tm) <- rownames(cd.mat)
  #   res <- template
  #   res[as.integer(names(tm))] <- tm
  #   return(res)
  # })
  # names(cd.target) <- names(target.nbhd)
  ...
  if(optns$deriv == 0 & is.null(optns$wt.fn)){
    ...
    res.locMetric <- mapply(
      function(obsv, i.tgt){#, tgt, i.tgt){

        if(n.loc.obsv[i.tgt] < min.n.local)
          return(list(NULL, error = simpleError('insufficient data')))
        ...
        return(
          tryCatch(
            do.call(locMetric, ls.args)
            , error = function(e) list(NULL, error = e)
          )
        )
  ...
  if(!optns$tensor.only){
    ...
    loc.res$error <- lapply(res.locMetric, `[[`, 'error')
  }
...
#'    }
#'   \item{\code{christ}}{
#'     estimated (1, 2)-tensor of Christoffel symbol, if requested. Indexed as
#'     \code{christ[i, j, k] = }\eqn{\Gamma_{ij}^k}.
#'    }
#'   \item{\code{curvature}}{
#'     estimated (0, 4)-curvature tensor if requested. Indexed as
#'     \code{curvature[i, j, k, l] = }\eqn{R_{ijkl}}.
#'    }
#'   \item{\code{intercept}}{
...
#' of either endpoint subtracted by the coordinate of the target point, while
...
#'    }
#' }
#' One can also use \code{\link{set_optns}} to set those options.
#'
#' @export
#'
#' @family {locMetric}
locMetric <- function(y, ls.cd, optns = list()){
  ...
  if(optns$type != "compare"){
    ...
    # # possible weighting
    # if(!is.null(optns$wt.fn)){
    #   arr.wt <- do.call(optns$wt.fn, ls.cd)
    # }else{
    #   arr.wt <- NULL
    # }
    ...
    # if(!is.null(optns$wt.fn)){
    #   arr.wt <-
    #     do.call(optns$wt.fn, ls.cd[[1]]) * do.call(optns$wt.fn, ls.cd[[2]])
    # }else{
    #   arr.wt <- NULL
    # }
  ...
  if(optns$intercept == TRUE){
    ...
    mat.design <- cbind(
      data.frame(intercept = rep(1, nrow(mat.design))),
      mat.design
    )
  }
  ...
  loc.fit <- #tryCatch(
    do.call(eval(parse(text = use.fn)), c(
  ...
  if(!is.null(optns$proj.pd)){
    if(optns$proj.pd) res$metric <- mat2PD(res$metric)
  }
...
locMetric_old <- function(y, coord.diff, coord.diff.target, optns = list()){
  ...
  if(optns$type != "compare"){
    ...
    # possible weighting
    if(!is.null(optns$wt.fn)){
      arr.wt <- optns$wt.fn(coord.diff, coord.diff.target)
    }else{
      arr.wt <- NULL
    }
  ...
  if(!is.null(optns$family)){# if using glm.fit with general family
    ...
    # loc.fit <- NULL
    # loc.fit <- do.call(stats::lm, c(
    #   list(formula = y ~ 0 + ., data = quote(loc.dat)),
    #   optns[idx.optns.glm]
    # ))
  ...
  loc.fit$call <- NULL
  # str.call <- deparse(loc.fit$call)
  # str.call <- paste(str.call, collapse = '')
  # str.call <-
  #   stringr::str_remove_all(str.call, '\\(function \\(.*\\) \\{.*\\}\\)')
  # loc.fit$call <- base::str2lang(sprintf(
  #   '%s%s', use.fn, str.call
  # ))
...
#' Local Estimation of Metric and Curvature Tensor
#' @description
#'   Local estimation of Riemannian metric tensor and Riemannian (0, 4)
#'   curvature tensor with noisy geodesic distances.
#'
#' @param y
#'   array of response
#' @param coord.diff
...
#'   edge (connecting a pair of points). Number of rows = length of \code{y}.
#'   If comparing edges, then a list of 2 such matrices.
#' @param coord.diff.target
#'   a list of matrix of difference of coordinates between endpoints of edges to
#'   the targeted point where tensors are estimated.
#' @param optns
#'   a list of control options.
#'
#' @return a list of
...
#' }
#' @details
#' The order of subtraction in \code{coord.diff} and \code{coord.diff.target}
#' must be coherent, in a sense that
#' \code{coord.diff = end.point[1] - end.point[2]}
#' corresponds to
#' \code{coord.diff.target = list(end.point[1] - target, end.point[2] - target)}
#' where the \code{end.point[1]} and \code{end.point[2]} are the end points of
#' an edge.
#' \cr
#' Possible control options are
#' \describe{
...
locRiemCvt <- function(y, coord.diff, coord.diff.target, optns = list()){
  ...
  if(optns$type != "compare"){
    ...
    vec.cvt.basis <- as.matrix(df.cvt.idx[
      , stringr::str_detect(names(df.cvt.idx), 'tsr\\.e\\d+'), drop = F
    ])
  ...
  #   # trying to avoid serialization...
  #   loc.fit <- do.call(stats::lm, args = c(
  #     list(formula = y ~ 0 + ., data = quote(loc.dat)),
  #     optns[idx.optns.glm]
  #   ))
  #   # nope...
  #   # loc.fit <- do.call.tommy(stats::lm, args = c(
  #   #   list(formula = y ~ 0 + ., data = quote(loc.dat)),
  #   #   optns[idx.optns.glm]
  #   # ), quote = T)
  #   # loc.fit <- do.call.jeroen(stats::lm, args = c(
  #   #   list(formula = y ~ 0 + ., data = loc.dat),
  #   #   optns[idx.optns.glm]
  #   # ))
  #   vec.par <- loc.fit$coefficients
  #   # loc.fit <- NULL
  ...
  # str.call <- deparse(loc.fit$call)
  # str.call <- paste(str.call, collapse = '')
  # str.call <- str_remove_all(str.call, '\\(function \\(.*\\) \\{.*\\}\\)')
  # loc.fit$call <- base::str2lang(sprintf(
  #   '%s%s', use.fn, str.call
  # ))
